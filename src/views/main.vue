...
//        this.components.push(newComp)
      ...
      addSymbol(id, content, url) {
        if (!document.getElementById(id)) {
          ...
          this.compSymbolCopy = _.cloneDeep(this.compSymbols)
...
//        }
      ...
      onOpenLocalTpl(e) {
        ...
        reader.onload = (e) => {
          const tplStr = JSON.parse(e.target.result)
          tplStr.compSymbols.map(function(item) {
            axios.get(item.url).then((response) => {
              // console.log(response)
              item.content = response.data
            })
          })
          // console.log(tplStr.compSymbols)
